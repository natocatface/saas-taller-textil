...
<div class="max-w-7xl mx-auto space-y-6">
    <div class="flex items-center gap-2 text-sm text-slate-400">
        ...
        <span class="text-slate-600 font-medium">Proveedores</span>
    </div>

    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Proveedores</h1>
            <p class="text-slate-500">Directorio de proveedores de insumos.</p>
        </div>
        <a href="{{ route('proveedores.create') }}" class="rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-brand-600/30 hover:bg-brand-700">+ Nuevo proveedor</a>
    </div>
    ...
    <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-100 overflow-hidden">
        <div class="p-4 border-b border-slate-100">
            <form method="GET" class="relative max-w-sm">
                ...
                <input type="text" name="q" value="{{ $q }}" placeholder="Buscar por razón social o RUC…"
            ...
            <table class="w-full text-sm">
                <thead class="text-xs uppercase text-slate-400 bg-slate-50">
                    <tr>
                        ...
                        <th class="text-left font-semibold px-6 py-3">RUC</th>
                        <th class="text-left font-semibold px-6 py-3">Teléfono</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($proveedores as $p)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-6 py-3.5 text-slate-600">{{ $p->ruc ?: '—' }}</td>
                            <td class="px-6 py-3.5 text-slate-600">{{ $p->telefono ?: '—' }}</td>
                            <td class="px-6 py-3.5"><span class="text-xs font-semibold px-2.5 py-1 rounded-lg {{ $p->tipo==='importado' ? 'bg-amber-50 text-amber-600' : 'bg-brand-50 text-brand-600' }} capitalize">{{ $p->tipo }}</span></td>
                            ...
                            <td class="px-6 py-3.5">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('proveedores.edit', $p) }}" class="p-2 rounded-lg text-slate-400 hover:bg-brand-50 hover:text-brand-600" title="Editar">
                                    ...
                                    <form method="POST" action="{{ route('proveedores.destroy', $p) }}" onsubmit="return confirm('¿Eliminar este proveedor?')">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="6" class="px-6 py-16 text-center text-slate-400">No hay proveedores registrados. <a href="{{ route('proveedores.create') }}" class="text-brand-600 font-medium">Crear el primero</a></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        @if ($proveedores->hasPages())
            <div class="p-4 border-t border-slate-100">{{ $proveedores->links() }}</div>
        @endif
    </div>
</div>
@endsection
