...
<div class="max-w-7xl mx-auto space-y-6">
    <div class="flex items-center gap-2 text-sm text-slate-400">
        ...
        <span class="text-slate-600 font-medium">Pedidos</span>
    </div>

    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Pedidos</h1>
            <p class="text-slate-500">Órdenes de trabajo de los clientes.</p>
        </div>
        <a href="{{ route('pedidos.create') }}" class="rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-brand-600/30 hover:bg-brand-700">+ Nuevo pedido</a>
    </div>
            ...
            <table class="w-full text-sm">
                <thead class="text-xs uppercase text-slate-400 bg-slate-50">
                    <tr>
                        ...
                        <th class="text-left font-semibold px-6 py-3">Entrega</th>
                        <th class="text-center font-semibold px-6 py-3">Ítems</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($pedidos as $p)
                        <tr class="hover:bg-slate-50">
                            <td class="px-6 py-3.5">
                                <a href="{{ route('pedidos.show', $p) }}" class="font-semibold text-brand-600 hover:text-brand-700">{{ $p->codigo }}</a>
                                <p class="text-xs text-slate-400">{{ $p->fecha_pedido?->format('d/m/Y') }}</p>
                            </td>
                            <td class="px-6 py-3.5 text-slate-600">{{ $p->cliente->nombre ?? '—' }}</td>
                            <td class="px-6 py-3.5 text-slate-600">{{ $p->fecha_entrega?->format('d/m/Y') ?? '—' }}</td>
                            <td class="px-6 py-3.5 text-center text-slate-600">{{ $p->items_count }}</td>
                            ...
                            <td class="px-6 py-3.5">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('pedidos.show', $p) }}" class="p-2 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600" title="Ver">
                                    ...
                                    <a href="{{ route('pedidos.edit', $p) }}" class="p-2 rounded-lg text-slate-400 hover:bg-brand-50 hover:text-brand-600" title="Editar">
                                    ...
                                    <form method="POST" action="{{ route('pedidos.destroy', $p) }}" onsubmit="return confirm('¿Eliminar este pedido?')">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="7" class="px-6 py-16 text-center text-slate-400">No hay pedidos registrados. <a href="{{ route('pedidos.create') }}" class="text-brand-600 font-medium">Crear el primero</a></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
