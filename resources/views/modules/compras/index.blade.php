...
<div class="max-w-7xl mx-auto space-y-6">
    <div class="flex items-center gap-2 text-sm text-slate-400">
        ...
        <span class="text-slate-600 font-medium">Compras</span>
    </div>

    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Compras</h1>
            <p class="text-slate-500">Órdenes de compra a proveedores. Al marcar una como <span class="font-medium">Recibida</span> se suma el stock de insumos.</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('compras.export', ['q'=>$q,'estado'=>$estado]) }}" class="rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-emerald-600/30 hover:bg-emerald-700 inline-flex items-center gap-2">
            ...
            <a href="{{ route('compras.create') }}" class="rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-brand-600/30 hover:bg-brand-700">+ Nueva compra</a>
        </div>
    </div>
    ...
    <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-100 overflow-hidden">
        <div class="p-4 border-b border-slate-100">
            <form method="GET" class="flex flex-col sm:flex-row gap-3">
                <div class="relative flex-1 max-w-sm">
                    ...
                    <input type="text" name="q" value="{{ $q }}" placeholder="Buscar por código o proveedor…"
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="text-xs uppercase text-slate-400 bg-slate-50">
                    <tr>
                        <th class="text-left font-semibold px-6 py-3">Compra</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($compras as $c)
                        <tr class="hover:bg-slate-50">
                            <td class="px-6 py-3.5">
                                <a href="{{ route('compras.show', $c) }}" class="font-semibold text-brand-600 hover:text-brand-700">{{ $c->codigo }}</a>
                                <p class="text-xs text-slate-400">{{ $c->fecha?->format('d/m/Y') }}</p>
                            </td>
                            <td class="px-6 py-3.5 text-slate-600">{{ $c->proveedor->razon_social ?? '—' }}</td>
                            <td class="px-6 py-3.5">
                                <span class="text-xs font-semibold px-2.5 py-1 rounded-lg {{ $c->estadoColor() }}">{{ \App\Models\Compra::ESTADOS[$c->estado] }}</span>
                                @if($c->stock_aplicado)<span class="text-[10px] text-emerald-500 ml-1">stock ✓</span>@endif
                            </td>
                            <td class="px-6 py-3.5 text-right font-semibold text-slate-700">S/ {{ number_format($c->total, 2) }}</td>
                            <td class="px-6 py-3.5">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('compras.show', $c) }}" class="p-2 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600" title="Ver">
                                    ...
                                    <a href="{{ route('compras.edit', $c) }}" class="p-2 rounded-lg text-slate-400 hover:bg-brand-50 hover:text-brand-600" title="Editar">
                                    ...
                                    <form method="POST" action="{{ route('compras.destroy', $c) }}" onsubmit="return confirm('¿Eliminar esta compra? Si tenía stock aplicado, se revertirá.')">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="5" class="px-6 py-16 text-center text-slate-400">No hay compras registradas. <a href="{{ route('compras.create') }}" class="text-brand-600 font-medium">Registrar la primera</a></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        @if ($compras->hasPages())
            <div class="p-4 border-t border-slate-100">{{ $compras->links() }}</div>
        @endif
    </div>
</div>
@endsection
